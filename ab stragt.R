...
paper_fun <- function(Noun_list,Points_list){
  ...
  paste(sample(nouns,1),"lies at the heart of the discussion on", sample(nouns,1),".")
  )
  ...
  paste("There is a rapidly growing literature on", sample(nouns,1), "which indicates that", sample(points,1),"."),
  ...
  # Resume work here
  Premise <- c( 
  paste("The main theoretical premise behind", sample(nouns,1),"is that", sample(points,1),"."),
  paste(sample(nouns,1),"and",sample(nouns,1), "share an important premise:", sample(points,1),"."),
  paste(sample(nouns,1),"is premised on the assumption that",sample(points,1),"."),
  paste("The basic premises of", sample(authors,1),"'s theory are", sample(points,1),",",sample(points,1),", and",sample(points,1),"."),
  paste("The arguments against", sample(authors,1), "’s premise rest on two assumptions:", sample(points,1),", and",sample(points,1),".")
  )
  ...
  View <- c(
  paste("The consensus view seems to be that",sample(points,1),"."),
  paste(sample(authors,1),"propounds the view that",sample(points,1),"."),
  paste("Current research",sample(c("does", "does not"),1), "appear to validate such a view."),
  paste("There have been dissenters to the view that",sample(points,1),"."),
  paste("The difference between",sample(nouns,1),"and", sample(nouns,1),"is not as clear-cut as popular views might suggest."),
  paste("The view that", sample(points,1),"is in line with",sample(points,1),"."),
  paste("I am not alone in my view that", sample(points,1),"."),
  paste(sample(authors,1),"puts forward the view that",sample(points,1),"."),
  paste(sample(authors,1),"s views rest on the assumption that",sample(points,1),".")
  )
...
paper_fun(Noun_list = c("Toads","Frogs","Salamanders"),Points_list = c("shits crazy bud"))
